export default class PictureEditWebSocket {
   ...
   */
  connect() {
    // 直连 picture-service WebSocket，绕过 Gateway 避免路由配置问题
    const BASE_URL = "ws://localhost:8203";
    // 从 localStorage 获取 JWT token
    const token = localStorage.getItem('authToken')
    const tokenParam = token ? `&token=${token}` : ''
    const url = `${BASE_URL}/ws/picture/edit?pictureId=${this.pictureId}${tokenParam}`
