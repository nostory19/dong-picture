...
import static com.dong.dongpicturebackend.constant.UserConstant.USER_LOGIN_STATE;
 ...
 * @DESC: 自定义权限加载接口实现类
 */
// component注解，保证此类被SpringBoot扫描，完成Sa-Token的自定义权限验证扩展
@Component
public class StpInterfaceImpl implements StpInterface {
    ...
    public SpaceUserAuthContext getAuthContextByRequest() {
        ...
        // 获取请求参数
            // 如果是json，则反序列化为authContext
        if (ContentType.JSON.getValue().equals(contentType)){
            //===================!!!!!!!!!!!!!!!!!!!!============
            // 注意这里ServletUtil.getBody(request)，
            // 虽然获取到了参数消息，但是HttpServletRequest的body值是个流，只支持读取一次，读完就没了
            // 其他地方读取就是空的了，为了解决这个问题，需要在config包下自定义请求包装类和请求包装类过滤器
